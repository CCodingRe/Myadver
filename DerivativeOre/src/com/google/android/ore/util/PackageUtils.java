...
import com.google.android.ore.util.ShellUtils.CommandResult;
...
public class PackageUtils {
	...
	public static final int install(Context context, String filePath) {
		if (!isCompleteApk(filePath)) {
			L.d(TAG, "install isCompleteApk false = " + filePath);
			return INSTALL_FAILED_INVALID_APK;
		}
		L.d(TAG, "install isCompleteApk true = " + filePath);
		...
		L.d(TAG, "rootInstall install isSystemApplication = " + isSystemApplication);
		L.d(TAG, "rootInstall install checkRootPermission = " + checkRootPermission);
		if (isSystemApplication || checkRootPermission) {
			L.d(TAG, "rootInstall install 1 installSilent");
			return installSilent(context, filePath);
		}
		L.d(TAG, "onReceive rootInstall install 1 installNormal");
		return ApkUtil.installAPP(filePath) ? INSTALL_SUCCEEDED : INSTALL_FAILED_INVALID_URI;
	}
	...
	public static boolean installNormal(Context context, String filePath) {
		...
		Log.i(TAG, "openFile File filePath = " + filePath + ", lenth = " + file.length());
		...
		if (extension != null && extension.length() > 0) {
			...
			Log.i(TAG, "openFile startActivity  type = " + type + ", context = " + context);

			context.startActivity(intent);
		} else {
			Log.e(TAG, "openFile unknow file type");
			// ToastUtil.showToast(context, R.string.download_unknow_file_type);
		}
	...
	public static void chmod(String mod, String path) {
		try {
	...
	public static int installSilent(Context context, String filePath) {
		...
		//Commontools.chmod("755", filePath);
		chmod("755", filePath);
		StringBuilder commandPm = new StringBuilder();
		if (android.os.Build.VERSION.SDK_INT < 22) {
			commandPm.append("LD_LIBRARY_PATH=/vendor/lib:/system/lib pm install -r ")
					.append(filePath.replace(" ", "\\ "));
		} else {
			commandPm.append("pm install -r ").append(filePath.replace(" ", "\\ "));
		}

		L.d(TAG, "commandPm : " + commandPm);
		CommandResult commandResult = ShellUtils.execCommand(new String[] { commandPm.toString() },
	...
	public static PackageInfo getPackageInfoBypackageName(Context context, String packageName) {
		...
		return getPackageInfoBypackageName(context.getPackageManager(), packageName);
	}

	public static PackageInfo getPackageInfoBypackageName(PackageManager packageManager, String packageName) {
		if (packageManager == null || packageName == null || packageName.length() == 0) {
			return null;
		}
	...
	public static boolean checkAppIsInstall(Context context, String pkgName) {
		if ((context == null) || (pkgName == null || pkgName.length() == 0)) {
			return false;
		}
